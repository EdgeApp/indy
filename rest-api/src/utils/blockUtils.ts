...
export async function getTransactions (block, address) : Promise<Array<Transaction>> {
  try {
    if (block && block.transactions) {
      ...
      resTransactions = address ?
        resTransactions.filter((transaction) => (transaction.from.toLower() === address.toLower() || transaction.to.toLower() === address.toLower())) : resTransactions
